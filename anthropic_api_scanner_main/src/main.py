...
class APIKeyLeakageScanner:
    ...
    def __init__(self, db_file: str, keywords: list, languages: list, headless: bool = False):
        ...
        rich.print(f"📂 Opening database file anthropic_db.db")

        self.dbmgr = DatabaseManager('anthropic')
    ...
    def check_api_keys_and_save(self, keys: list[str]):
        ...
        check_key_anthropic = partial(check_key, service='anthropic')
        with ThreadPoolExecutor(max_workers=10) as executor:
            results = list(executor.map(check_key_anthropic, unique_keys))
            with self.dbmgr as mgr:
                ...
                mgr.export_to_csv(csv_path, service='anthropic')
    ...
    def update_existed_keys(self):
        ...
        with self.dbmgr as mgr:
            ...
            for key in tqdm(keys, desc="🔄 Updating existed keys ..."):
                result = check_key_anthropic(key[0])
                mgr.delete(key[0])
