...
log = logging.getLogger("Grok-API-Leakage")
httpx_logger = logging.getLogger("httpx")
httpx_logger.setLevel(logging.WARNING)

class APIKeyLeakageScanner:
    """
    Scan GitHub for available Grok API Keys
    """
    def __init__(self, db_file: str, keywords: list, languages: list):
        ...
        self.dbmgr = DatabaseManager('grok')
    ...
    def login_to_github(self):
        ...
        if not cookie_exists:
            ...
            # input("Press Enter after you logged in: ")  # For parallel, skip input
            self.cookies.save()
        else:
    ...
    def _process_url(self, url: str):
        ...
        while True:
            ...
            rich.print(f"    🌕 There are {len(urls_need_expand)} urls waiting to be expanded for [bold yellow]{search_key}[/bold yellow]")
            try:
                next_buttons = self.driver.find_elements(by=By.XPATH, value="//a[@aria-label='Next Page']")
                rich.print(f"🔍 Clicking next page for [bold yellow]{search_key}[/bold yellow]")
                WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.XPATH, "//a[@aria-label='Next Page']")))
                next_buttons = self.driver.find_elements(by=By.XPATH, value="//a[@aria-label='Next Page']")
                next_buttons[0].click()
            except Exception:
                rich.print(f"⚪️ No more pages for [bold yellow]{search_key}[/bold yellow]")
                break
    ...
    def check_api_keys_and_save(self, keys: list[str]):
        ...
        check_key_grok = partial(check_key, service='grok')
        with ThreadPoolExecutor(max_workers=10) as executor:
            results = list(executor.map(check_key_grok, unique_keys))
            with self.dbmgr as mgr:
...
def main(from_iter: int | None = None, check_existed_keys_only: bool = False, keywords: list | None = None, languages: list | None = None, check_insuffcient_quota: bool = False):
    ...
    leakage = APIKeyLeakageScanner("grok_db.db", keywords, languages)
    try:
